...
function scene:show( event )
  ...
  if ( phase == "will" ) then
    ...
    local mediumWordsValues = jsonFunct.readMediumWords()
    local mediumLettersValues = jsonFunct.readMediumLetters()
    local mediumWordsHintsValues = jsonFunct.readMediumWordsHints()
    local mediumAnswerLetters = jsonFunct.readMediumAnswerLetters()
    local mediumLevelStats = jsonFunct.readMediumStatsValues()
    ...
    local level = currentLevelValues[5]
    local levelWords = mediumWordsValues[level]
    local levelLetters = mediumLettersValues[level]
    local levelWordsHints = mediumWordsHintsValues[level]
    local levelAnswerLetters = mediumAnswerLetters[level]
    ...
    local candyCount=1
    local candyXPos = {dW*.62, dW*.47, dW*.32, dW*.17}
    ...
    local function gameWon()
      ...
      local function checkPlusCoin()
        if level%5 == 0 and mediumLevelStats[level][1] == 0  then
          images.plusOneCoin.y = images.plusOneCoin.y - dH
          coins = coins + 1
          gameValues[1] = coins
          jsonFunct.writeGameValues(gameValues)
        ...
        if totalScore > mediumLevelStats[level][1] then
          mediumLevelStats[level][1] = totalScore
          mediumLevelStats[level][2] = moves
          mediumLevelStats[level][3] = timerVar
          mediumLevelStats[level][4] = starCounter
          jsonFunct.writeMediumStatsValues(mediumLevelStats)
        end
      end
      ...
      if level == currentLevelValues[2] and level < 300 then
        currentLevelValues[2] = currentLevelValues[2]+1
        table.insert(mediumLevelStats,{0,0,0,0})
        jsonFunct.writeMediumStatsValues(mediumLevelStats)
    ...
    local function checkWords()
      ...
      local fourWord4 = levelLetters[13] .. levelLetters[14] .. levelLetters[15] .. levelLetters[16]
      local fourWord5 = levelLetters[1] .. levelLetters[5] .. levelLetters[9] .. levelLetters[13]
      local fourWord6 = levelLetters[2] .. levelLetters[6] .. levelLetters[10] .. levelLetters[14]
      local fourWord7 = levelLetters[3] .. levelLetters[7] .. levelLetters[11] .. levelLetters[15]
      local fourWord8 = levelLetters[4] .. levelLetters[8] .. levelLetters[12] .. levelLetters[16]
      ...
      local threeWord7 = levelLetters[13] .. levelLetters[14] .. levelLetters[15]
      local threeWord8 = levelLetters[14] .. levelLetters[15] .. levelLetters[16]
      local threeWord9 = levelLetters[1] .. levelLetters[5] .. levelLetters[9]
      local threeWord10 = levelLetters[5] .. levelLetters[9] .. levelLetters[13]
      local threeWord11 = levelLetters[2] .. levelLetters[6] .. levelLetters[10]
      local threeWord12 = levelLetters[6] .. levelLetters[10] .. levelLetters[14]
      local threeWord13 = levelLetters[3] .. levelLetters[7] .. levelLetters[11]
      local threeWord14 = levelLetters[7] .. levelLetters[11] .. levelLetters[15]
      local threeWord15 = levelLetters[4] .. levelLetters[8] .. levelLetters[12]
      local threeWord16 = levelLetters[8] .. levelLetters[12] .. levelLetters[16]
      ...
      local twoWord10 = levelLetters[13] .. levelLetters[14]
      local twoWord11 = levelLetters[14] .. levelLetters[15]
      local twoWord12 = levelLetters[15] .. levelLetters[16]
      local twoWord13 = levelLetters[1] .. levelLetters[5]
      local twoWord14 = levelLetters[5] .. levelLetters[9]
      local twoWord15 = levelLetters[9] .. levelLetters[13]
      local twoWord16 = levelLetters[2] .. levelLetters[6]
      local twoWord17 = levelLetters[6] .. levelLetters[10]
      local twoWord18 = levelLetters[10] .. levelLetters[14]
      local twoWord19 = levelLetters[3] .. levelLetters[7]
      local twoWord20 = levelLetters[7] .. levelLetters[11]
      local twoWord21 = levelLetters[11] .. levelLetters[15]
      local twoWord22 = levelLetters[4] .. levelLetters[8]
      local twoWord23 = levelLetters[8] .. levelLetters[12]
      local twoWord24 = levelLetters[12] .. levelLetters[16]
      ...
      local function checkFours(word, parsedWord, wordNum, acrOrDown, startNum, yPos)
        if word == parsedWord then
          ...
          for v=0, 3, 1 do
            ...
            greenCandyTable[startNum+4].y = candyYPos[3]-dH
            greenCandyTable[startNum+8].y = candyYPos[2]-dH
            greenCandyTable[startNum+12].y = candyYPos[1]-dH
        end
      ...
      local function checkThrees(word, parsedWord, wordNum, acrOrDown, startNum, yPos)
        if word == parsedWord then
          if acrOrDown==0 then
            ...
            greenCandyTable[startNum+8].y = candyYPos[yPos-2]-dH
          end
          levelWordsTable[wordNum]:setFillColor(.1,.95,.1)
          wordTracker[wordNum]=1
        end
      ...
      for c=#levelWords,1,-1 do
        if #levelWords[c] == 4 then
          ...
          checkFours(levelWords[c], fourWord2, c, 0, 5, 3)
          checkFours(levelWords[c], fourWord3, c, 0, 9, 2)
          checkFours(levelWords[c], fourWord4, c, 0, 13, 1)
          checkFours(levelWords[c], fourWord5, c, 1, 1)
          checkFours(levelWords[c], fourWord6, c, 1, 2)
          checkFours(levelWords[c], fourWord7, c, 1, 3)
          checkFours(levelWords[c], fourWord8, c, 1, 4)
        end
        if #levelWords[c] == 3 then
          checkThrees(levelWords[c], threeWord1, c, 0, 1, 4)
          checkThrees(levelWords[c], threeWord2, c, 0, 2, 4)
          checkThrees(levelWords[c], threeWord3, c, 0, 5, 3)
          checkThrees(levelWords[c], threeWord4, c, 0, 6, 3)
          checkThrees(levelWords[c], threeWord5, c, 0, 9, 2)
          checkThrees(levelWords[c], threeWord6, c, 0, 10, 2)
          checkThrees(levelWords[c], threeWord7, c, 0, 13, 1)
          checkThrees(levelWords[c], threeWord8, c, 0, 14, 1)
          checkThrees(levelWords[c], threeWord9, c, 1, 1, 4)
          checkThrees(levelWords[c], threeWord10, c, 1, 5, 3)
          checkThrees(levelWords[c], threeWord11, c, 1, 2, 4)
          checkThrees(levelWords[c], threeWord12, c, 1, 6, 3)
          checkThrees(levelWords[c], threeWord13, c, 1, 3, 4)
          checkThrees(levelWords[c], threeWord14, c, 1, 7, 3)
          checkThrees(levelWords[c], threeWord15, c, 1, 4, 4)
          checkThrees(levelWords[c], threeWord16, c, 1, 8, 3)
        end
        if #levelWords[c] == 2 then
          ...
          checkTwos(levelWords[c], twoWord4, c, 0, 5, 3)
          checkTwos(levelWords[c], twoWord5, c, 0, 6, 3)
          checkTwos(levelWords[c], twoWord6, c, 0, 7, 3)
          checkTwos(levelWords[c], twoWord7, c, 0, 9, 2)
          checkTwos(levelWords[c], twoWord8, c, 0, 10, 2)
          checkTwos(levelWords[c], twoWord9, c, 0, 11, 2)
          checkTwos(levelWords[c], twoWord10, c, 0, 13, 1)
          checkTwos(levelWords[c], twoWord11, c, 0, 14, 1)
          checkTwos(levelWords[c], twoWord12, c, 0, 15, 1)
          checkTwos(levelWords[c], twoWord13, c, 1, 1, 4)
          checkTwos(levelWords[c], twoWord14, c, 1, 5, 3)
          checkTwos(levelWords[c], twoWord15, c, 1, 9, 2)
          checkTwos(levelWords[c], twoWord16, c, 1, 2, 4)
          checkTwos(levelWords[c], twoWord17, c, 1, 6, 3)
          checkTwos(levelWords[c], twoWord18, c, 1, 10, 2)
          checkTwos(levelWords[c], twoWord19, c, 1, 3, 4)
          checkTwos(levelWords[c], twoWord20, c, 1, 7, 3)
          checkTwos(levelWords[c], twoWord21, c, 1, 11, 2)
          checkTwos(levelWords[c], twoWord22, c, 1, 4, 4)
          checkTwos(levelWords[c], twoWord23, c, 1, 8, 3)
          checkTwos(levelWords[c], twoWord24, c, 1, 12, 2)
        end
      end
    ...
    images.background = createImage(bgGroup, "images/background2.png", dW*2, dH, cX, cY)
    images.backBox = display.newRoundedRect(bgGroup, dW*.392, cY, dW*.65, dH*.98, 40  )
    images.backBox.fill = {.4, .4}
    images.backBoxWords = display.newRoundedRect(bgGroup, dW*.87, cY, dW*.22, dH*.96, 15)
    images.backBoxWords.fill = {.4, .4}
    images.pauseButton = createImage(bgGroup, "images/pauseButton.png", 45, 45, dW*.925, dH*.11 )
    images.hintButton = createImage(bgGroup, "images/hintButton.png", 45, 45, dW*.82, dH*.11)
    images.actualGameTimer = createWords(bgGroup, timerVar, dW*.87, dH*.24, 30 )
    ...
    for e = 1, #levelWords, 1 do
      local actualLevelWords = createWords(candyWordsGroup, levelWords[e], dW*.876, (dH*.99)-(e*(dH*.74)/(#levelWords+1))+dH, 20 )
      ...
      elseif ( phase == "ended" ) then
        ...
        elseif target == images.gameWonNextButton and windowName == "gameWonWindow" then
          if lives > 0 then
            screenName = "code.playMedium"
            handleInterstellar()
            if currentLevelValues[5] < 300 then
              currentLevelValues[5] = currentLevelValues[5] + 1
              jsonFunct.writeCurrentLevelValues(currentLevelValues)
            end
    ...
    local function startGameTimer()
      ...
      if storeValues[3] == 1 then
        gameTimerDelay = timer.performWithDelay( 1000, gameTimer, 240)
      else
        gameTimerDelay = timer.performWithDelay( 1000, gameTimer, 120)
      end
    end
    ...
    function scene:processReward()
      ...
      gameValues[2] = lives
      jsonFunct.writeGameValues(gameValues)
      composer.gotoScene( "code.playMedium"  )
    end
  end
